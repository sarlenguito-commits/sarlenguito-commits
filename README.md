### ¡Hola! Soy Esteban 👋

Me estoy formando como **Desarrollador Full Stack**, con foco en **Automatización con IA** e **IA Engineering**. Curso la carrera en Coderhouse y sigo estudiando por mi cuenta en paralelo.

- 🌱 Actualmente cursando: Desarrollo Web (carrera Full Stack)
- 📍 Neuquén, Argentina

---

### 🚀 Proyectos destacados

**[CS2database](https://github.com/sarlenguito-commits/CS2database)** · [ver web](https://cs2databasetnl.onrender.com)
Plataforma tipo CRM de jugadores de CS2, con sync automático desde las APIs de FACEIT y Steam cada 4hs. Backend y frontend en Render, base de datos Postgres en Neon.

**[Sistema de Scouting Automatizado — CS2](https://github.com/sarlenguito-commits/sistema-scouting-cs2)**
Ecosistema de automatización con IA (n8n + Airtable + Gemini + WhatsApp) para reclutar jugadores de CS2, con punto de pausa humano (HITL) antes de acciones críticas. Proyecto Final del curso de Automatización con IA — Aprobado con 72%.

**[Poppy Pastelería](https://github.com/sarlenguito-commits/Poppy-Pasteleria-Web-)**
Sitio web estático de repostería artesanal para mascotas — HTML5, CSS3 (Grid, Flexbox) y Bootstrap 5. Proyecto final del curso de Desarrollo Web.

**[Calculadora Saludable](https://github.com/sarlenguito-commits/calculadora-saludable)**
Calculadora web de IMC, calorías y macros con recetas sugeridas según objetivo. HTML, CSS y JS.

---

### 📜 Certificaciones

- [**AI Automation**](./certificado-ai-automation.pdf) — Coderhouse (certificado por StudioChat) — 16/09/2026

---

### 🛠️ Stack

`HTML` `CSS` `JavaScript` `Bootstrap` `n8n` `Airtable` `Node.js` `Postgres` `Git/GitHub`
